...
async function atualizarDadosInterface() {
    ...
    try {
        ...
        // CORREÇÃO: Link sem chaves extras
...
window.onload = async () => {
    renderShop();
    const vGain = document.getElementById('visualGain');
    if (vGain) vGain.innerText = visualBalance.toFixed(6);
    if (window.ethereum) {
        const accounts = await window.ethereum.request({ method: 'eth_accounts' });
        if (accounts.length > 0) {
            userAccount = accounts[0];
            const wDisp = document.getElementById('walletDisplay');
            if (wDisp) wDisp.innerText = userAccount.substring(0, 6) + "...";
            await atualizarDadosInterface();
            if (lastActivation && (parseInt(lastActivation) + 86400000) > Date.now()) startMiningVisuals();
        }
    }
